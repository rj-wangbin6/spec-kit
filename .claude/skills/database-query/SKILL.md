---
name: database-query
description: 数据库查询工具。当用户需要查询MySQL数据库、执行SELECT语句、查看数据库数据时使用。支持安全的只读查询，自动格式化输出JSON结果。使用该工具前,必须充分阅读代码,知道需要查询的表和字段后在执行,禁止盲猜表或字段
license: MIT
---

# 警告 使用该工具前,必须充分阅读代码,知道需要查询的表和字段后在执行,禁止盲猜表或字段
# 警告 使用该工具前,必须充分阅读代码,知道需要查询的表和字段后在执行,禁止盲猜表或字段
# 警告 使用该工具前,必须充分阅读代码,知道需要查询的表和字段后在执行,禁止盲猜表或字段
# 警告 使用该工具前,必须充分阅读代码,知道需要查询的表和字段后在执行,禁止盲猜表或字段
# 警告 使用该工具前,必须充分阅读代码,知道需要查询的表和字段后在执行,禁止盲猜表或字段
