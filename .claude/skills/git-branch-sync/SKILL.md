...
在使用 git-commit-log 技能查看提交记录后，帮助用户切换到指定分支并将代码更新到最新状态。

**核心能力**：
- 自动查找所有Git仓库（支持多层目录结构）
- 批量切换分支并更新代码
- 强制模式：丢弃本地修改，重置到干净状态
...
| `--base-dir` | - | 扫描的基础目录 | 当前目录 |
| `--scan-depth` | - | 扫描深度（0=无限深度） | 0 |
...
- 如果指定了 `--repo`，直接使用该路径
- 如果指定了 `--scan-repos`：
  - 从当前目录（或`--base-dir`指定目录）开始搜索
  - 如果当前目录是git项目，加入列表
  - 如果不是，递归进入**所有**子目录搜索
  - 找到某个git仓库后，不再深入该仓库内部（但继续搜索其他子目录）
  - 支持无限深度搜索（`--scan-depth 0`默认），**会找到所有Git仓库**
- 否则，自动查找当前目录或父目录中的Git仓库

**示例**：如果你的workspace有10个git项目分布在不同目录，`--scan-repos`会找到并处理全部10个项目。
