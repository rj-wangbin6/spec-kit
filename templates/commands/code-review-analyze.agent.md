...
- 若父代理提供了 `02-change-collection.md` 或其他上游文件路径，必须先读取这些文件，再开始调用链追踪
- 若存在需求相关输入文件，也可一并读取，但仍不得输出质量判断
- 本子代理的输出必须保持结构稳定，便于父代理按范围分别保存为 `05-path-<scope>.md`
- 后续规则审查将直接读取这些路径文件，因此输出中必须保留清晰的范围标识、文件位置和调用路径
...
#### C项目追踪路径
```
main/命令入口 → 业务分发函数 → 模块实现函数 → 第三方边界或系统调用
```

**追踪关键点**：
- 注意条件编译、宏开关、头文件声明和函数定义分离的情况
- 优先识别 `include/` 中对外接口，再回溯到 `src/` 实现
- 若调用进入 `opensources/` 等第三方目录，只记录进入点和返回点
...
- 代码起止行号
- 核心代码片段（10-20 行）
- 调用类型（Feign/HTTP/本地/数据库/API/函数调用/第三方边界）
- 业务逻辑描述（客观陈述）
- 【如有需求】与需求的对应关系
...
若遇到第三方代码，只追踪业务代码与第三方的边界接口，不对第三方实现做质量评论。
