...
Every `specify init` run also executes a post-init hook in force-update mode, so installing or re-running Spec Kit keeps `git-ai` pinned to the latest release published from `rj-gaoang/git-ai` instead of only filling in a missing install. By default the hook downloads `install.ps1` or `install.sh` from `https://github.com/rj-gaoang/git-ai/releases/latest/download/` and exports `GIT_AI_GITHUB_REPO=rj-gaoang/git-ai` plus `GIT_AI_RELEASE_TAG=latest` before invoking the installer. You can still override the source with `GIT_AI_INSTALLER_URL`, switch repositories or tags with `GIT_AI_GITHUB_REPO` and `GIT_AI_RELEASE_TAG`, or validate an unreleased local build with `GIT_AI_LOCAL_BINARY`. The hook also refreshes `git-ai install-hooks` and emits troubleshooting logs for installer source selection, command resolution, and hook refresh decisions. On Windows, Spec Kit prefers `pwsh` and automatically falls back to Windows PowerShell (`powershell.exe`) when PowerShell 7 is not installed.

When a project uses the generated `upload-ai-stats.ps1` helper, all matching commits are uploaded in a single batch request. The remote request and response now use Java-friendly camelCase DTO fields, the raw git-ai tool/model map is normalized into `stats.toolModelBreakdown[]` entries such as `{ tool, model, aiAdditions, ... }`, each commit can also include file-level detail in `stats.files[]`, and `-Json` mode suppresses progress logs so the output stays machine-parseable. Build output paths under `target/` are excluded from `stats.files[]` by default; if your team needs a different omit list, set `GIT_AI_UPLOAD_EXCLUDE_PATH_PREFIXES` to a comma- or semicolon-separated prefix list.
...
On project initialization or refresh (`specify init --here --force`), Spec Kit also runs the generated post-init script to install or update `git-ai` from the latest release published by `rj-gaoang/git-ai`, then refresh repository hooks. The init flow now leaves behind explicit skip and failure warnings when the post-init hook cannot be launched. When `--script ps` is used on Windows, the launcher will use `pwsh` if available and fall back to `powershell.exe` otherwise.

The generated `upload-ai-stats.ps1` helper batches all selected commits into one remote request instead of posting once per commit, while still preserving per-commit success and failure reporting. The upload DTO now uses camelCase fields such as `repoUrl`, `commitSha`, and `hasAuthorshipNote`, converts the raw `tool_model_breakdown` map into a Java-friendly `toolModelBreakdown` list, nests per-file landed-diff detail under `stats.files[]`, keeps `-Json` output free of progress chatter for downstream parsing, and filters `target/` build artifacts out of `stats.files[]` by default.
