...
Every `specify init` run also executes a post-init hook that will install `git-ai` when missing, re-run the official installer when `--force` is used, refresh `git-ai install-hooks`, and set `git-ai config prompt_storage notes` so prompt text remains available in git notes for downstream upload flows. The hook now emits troubleshooting logs for installer source selection, command resolution, hook refresh decisions, and prompt-storage configuration. On Windows, Spec Kit prefers `pwsh` and automatically falls back to Windows PowerShell (`powershell.exe`) when PowerShell 7 is not installed.
...
On project initialization or refresh (`specify init --here --force`), Spec Kit also runs the generated post-init script to install `git-ai` through the official installer flow, refresh repository hooks, and switch `git-ai` prompt storage to `notes`. The init flow now leaves behind explicit skip and failure warnings when the post-init hook cannot be launched. When `--script ps` is used on Windows, the launcher will use `pwsh` if available and fall back to `powershell.exe` otherwise.
