...
function Rewrite-Paths {
    ...
    $Content = $Content -replace '(?m)(^|[\s`"(])/?memory/', '${1}.specify/memory/'
    $Content = $Content -replace '(?m)(^|[\s`"(])/?scripts/', '${1}.specify/scripts/'
    $Content = $Content -replace '(?m)(^|[\s`"(])/?templates/', '${1}.specify/templates/'
    return $Content
}
